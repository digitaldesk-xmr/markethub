...
const translations = {
    it: {
        ...
        nav_messages: "Messaggi", nav_upload: "Carica", nav_logout: "Esci", btn_login: "Accedi", btn_register: "Registrati",
        ...
        payment_choice: "Paga in crypto con NOWPayments:", all_products_title: "Tutti i prodotti",
        search_placeholder: "🔍 Cerca prodotti...", all_categories: "Tutte le categorie",
        cat_electronics: "Elettronica", cat_clothing: "Abbigliamento", cat_home: "Casa e Arredamento",
        cat_sport: "Sport", cat_collectibles: "Collezionismo", cat_other: "Altro", no_products: "Nessun prodotto in vendita.",
        ...
        buy_with_crypto: "💰 Compra con Crypto", contact_seller: "💬 Contatta venditore",
        no_reviews_yet: "⭐ Nessuna recensione", leave_review: "Lascia una recensione",
        your_comment: "Il tuo commento...", submit_review: "Invia recensione", seller_reply: "Risposta del venditore:",
        reply: "Rispondi", no_conversations: "Nessuna conversazione", chat_with: "💬 Chat con",
        type_message: "Scrivi un messaggio...", send: "Invia",
    ...
    en: {
        ...
        nav_messages: "Messages", nav_upload: "Upload", nav_logout: "Logout", btn_login: "Login", btn_register: "Sign Up",
        ...
        payment_choice: "Pay with crypto via NOWPayments:", all_products_title: "All Products",
        search_placeholder: "🔍 Search products...", all_categories: "All Categories",
        cat_electronics: "Electronics", cat_clothing: "Clothing", cat_home: "Home & Furniture",
        cat_sport: "Sports", cat_collectibles: "Collectibles", cat_other: "Other", no_products: "No products for sale.",
        ...
        buy_with_crypto: "💰 Buy with Crypto", contact_seller: "💬 Contact Seller",
        no_reviews_yet: "⭐ No reviews yet", leave_review: "Leave a review",
        your_comment: "Your comment...", submit_review: "Submit Review", seller_reply: "Seller reply:",
        reply: "Reply", no_conversations: "No conversations yet", chat_with: "💬 Chat with",
...
function updateNavbarForUser() {
    ...
    if (dashboardLink) dashboardLink.style.display = (isLoggedIn && hasActiveSub) ? 'inline-block' : 'none';
    if (uploadLink) uploadLink.style.display = (isLoggedIn && hasActiveSub) ? 'inline-block' : 'none';
    if (messagesLink) messagesLink.style.display = (isLoggedIn && hasActiveSub) ? 'inline-block' : 'none';
...
function checkPageAccess() {
    ...
    if (page.includes('upload-product.html') || page.includes('dashboard.html') || page.includes('messages.html')) {
        if (!currentUser) {
...
function loadDashboard() {
    ...
    let allReviews = [];
    userProducts.forEach(p => { if (p.reviews) p.reviews.forEach((r, idx) => allReviews.push({ productId: p.id, productName: p.name, review: r, reviewIndex: idx })); });
    ...
    if (reviewsContainer) {
        ...
        else reviewsContainer.innerHTML = allReviews.map(rev => `
    ...
    if (loginForm) loginForm.addEventListener('submit', (e) => {
        ...
        if (user) {
            ...
            if (userProducts.length > 0 && !checkSubscriptionStatus(email)) {
                alert(translations[currentLang].alert_subscription_expired);
                localStorage.removeItem('markethubUser');
                closeLogin();
                return;
            }
