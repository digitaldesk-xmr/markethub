...
async function loadDashboard() {
    ...
    userProducts.forEach(p => {
        if (p.reviews && p.reviews.length) {
            p.reviews.forEach((r, idx) => allReviews.push({ productId: p.id, productName: p.name, review: r, reviewIndex: idx }));
        }
    });
...
function setupUploadForm() {
    ...
    if (form) form.addEventListener('submit', async (e) => {
        ...
        const newId = await addProductToSupabase(newProduct);
        if (newId) {
            ...
            alert("Errore durante la pubblicazione su Supabase");
        }
    });
}
...
function getAverageRating(product) {
    ...
    let sum = product.reviews.reduce((acc, r) => acc + r.rating, 0);
